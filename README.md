...
|--------|-------------|-----------|
| Song Forge | ACE-Step 1.5 XL Turbo song generation, reference covers, source repaint/extend, stitched long-form songs, persistent retry/resume queue, and recovered vocal-stem export | ACE-Step, Demucs |
| Lyrics Engine | AI-powered lyrics writing with 33 genre templates, advisory rhyme/cadence feedback, and sortable history | Llama 3.1 8B |
...
| Vocal Suite | Singing synthesis, humming-to-MIDI lyric melody generation with verified Enhanced LRC export, consented voice-profile onboarding, selectable Demucs/MDX/MDXC/Roformer stem separation, target-DAW stem naming/export, and vocal auto-tune pitch correction | DiffSinger, Audio Separator |
...
Project Manager exports the open project's existing audio assets as a `.dawproject` archive, while Mixer exports the current track buffers with their volume, pan, mute, and solo state. Both paths run off the UI thread, use collision-safe media names, and reopen the archive through Slunder's structural validator before reporting success. Archive validation and extraction reject traversal, absolute-path, duplicate, symlink, and oversized entries; recoverable trash validates manifest paths before restoring anything. Project and job schemas retain unknown fields so newer files can round-trip through an older shell, while malformed roots are quarantined or reported through repair diagnostics. Vocal Suite remix export preserves the separated stems' native sample rate and records the source paths and rate in its provenance sidecar.
...
Model downloads and model-bearing inference workers use central, cancellation-aware admission limits (two downloads and one GPU-bound inference by default), so parallel requests queue safely while existing resumable downloads and SHA-256 verification remain unchanged.
...
The Voice Bank can store consented RVC and GPT-SoVITS profiles and auto-detects standard files, but conversion and cloning remain unavailable until verified local inference adapters are bundled. No placeholder audio is produced.
