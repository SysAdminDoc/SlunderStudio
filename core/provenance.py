...
REPLAYABILITY_STATES = frozenset({
    "byte_replayable",
    "semantic_replayable",
    "not_replayable",
})
...
@dataclass(frozen=True)
class ProvenanceReplayability:
    """Capability advertised for one provenance operation key."""
...
_REPLAYABILITY: dict[str, ProvenanceReplayability] = {}


def _provenance_key(provenance: dict[str, Any]) -> str:
    return str(
...
def provenance_replayability(provenance: dict[str, Any]) -> ProvenanceReplayability:
    """Return the current, fail-closed replay capability for a sidecar."""
    key = _provenance_key(provenance)
    capability = _REPLAYABILITY.get(key)
    if capability is not None:
        return capability
    return ProvenanceReplayability(
        key=key,
        state="not_replayable",
        reason=(
            f"No registered renderer is available for '{key or 'unknown operation'}'."
        ),
    )


def register_rerenderer(
    operation_key: str,
    renderer,
    *,
    replayability: str = "semantic_replayable",
    reason: str = "A renderer is registered, but regenerated bytes are not guaranteed identical.",
) -> None:
    ...
    state = str(replayability or "").strip()
    if state not in REPLAYABILITY_STATES or state == "not_replayable":
        raise ValueError(
            "Registered rerenderers must be byte_replayable or semantic_replayable"
        )
    clean_reason = " ".join(str(reason or "").split())
    if not clean_reason:
        raise ValueError("Registered rerenderers require a capability reason")
    _RERENDERERS[key] = renderer
    _REPLAYABILITY[key] = ProvenanceReplayability(key, state, clean_reason)
...
def write_provenance_sidecar(
    ...
    *,
    ...
    replayability = provenance_replayability({"rerender_key": rerender_key})
    payload = {
        ...
        "replayability": replayability.as_dict(),
...
def rerender_from_provenance(
    ...
    *,
    ...
    replayability = provenance_replayability(provenance)
    if replayability.state == "not_replayable":
        raise ProvenanceCompatibilityError(
            "Artifact is provenance-only and cannot be re-rendered",
            (
                ProvenanceDiff(
                    "replayability.state",
                    (provenance.get("replayability") or {}).get("state", ""),
                    replayability.state,
...
register_rerenderer(
    "sfx:generate",
    _rerender_sfx,
    replayability="byte_replayable",
    reason="Fixed seed, local model inputs, and the shared WAV writer are byte-compared after rerendering.",
)
register_rerenderer(
    "song_forge:generate",
    _rerender_ace_step,
    replayability="semantic_replayable",
    reason="Recorded ACE-Step inputs can be regenerated, but model/runtime variation can change output bytes.",
)
register_rerenderer(
    "song_forge:generate_long_form",
    _rerender_ace_step,
    replayability="semantic_replayable",
    reason="Recorded long-form sections can be regenerated, but model/runtime variation can change output bytes.",
)
register_rerenderer(
    "vocal_suite:vocal_autotune",
    _rerender_autotune,
    replayability="byte_replayable",
    reason="The recorded local pitch-correction operation is deterministic and verified by an artifact hash.",
)
register_rerenderer(
    "vocal_suite:diffsinger_pronunciation_correction",
    _rerender_diffsinger_pronunciation,
    replayability="semantic_replayable",
    reason="Recorded pronunciation overrides can be regenerated, but neural vocal output bytes are not guaranteed identical.",
)
...
def project_metadata_from_provenance(
    ...
    return {
        "provenance": {
            ...
            "replayability": provenance_replayability(provenance).as_dict(),
