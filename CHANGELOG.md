...
- Added DiffSinger pronunciation correction: select a rendered lyric unit or waveform region,
  enter explicit phoneme tokens, re-synthesize only that region, and crossfade it into the take
  while preserving all samples outside the edit. Overrides are recorded in provenance and open
  projects for reproducible recovery.
...
- Routed UI focus, selection, mood-curve, stem, SFX, and waveform colors through the approved
...
- Extended the piano-roll scene rect into its negative gutters so pitch labels and bar numbers
...
- Prevented failed batch and seed playback loads from replaying stale audio, routed load failures
  through application logging and an error toast, and made missing output sizes nonfatal.
- Preserved Lyrics Pro system-prompt edits through the high-level generation API and recorded the
...
- Made Model Hub cancellation interrupt Hugging Face transfers at progress checkpoints, kept cards in
  a visible stopping state while workers drain, reported Resume attempts, and preserved completion
...
- Trimmed model-backed SFX artifacts to the requested duration before normalization and WAV export.
- Aligned Whisper tiny loading and transcription with the registry's local Transformers snapshot,
  using only safetensors and no inference-time downloads.
- Preserved routed reference tempo and musical key in Song Forge generation tags and job inputs
...
- Debounced Vocal Suite readiness refreshes, skipped model checks for empty inputs, and cached
  verified readiness and Model Hub disk-usage scans until lifecycle status changes.
...
- Wired accessible names, descriptions, tab order, and widget-level focus indicators across the remaining twelve views, including keyboard-focus styling for graphics canvases and tab bars, with headless coverage for every visible focusable control.
